<template>
  <div class="ww-markdown">
    <div class="filler-object ww-markdown-edit-mode">
      <pre
        v-highlightjs="markdownText"
      ><code spellcheck='false' class='ww-markdown-box' :class='languageStyle'></code></pre>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    markdownText() {
      if (this.wwObject) {
        return this.content.data.markdownText;
      } else {
        return "";
      }
    },
    languageStyle() {
      if (this.wwObject) {
        return this.content.data.languageStyle;
      } else {
        return "";
      }
    },
  ...
  methods: {
    init() {
      ...
      if (!this.content.data.markdownText) {
        this.content.data.markdownText = wwLib.wwObject.getDefault({
          type: "ww-text",
        });
      }
      if (!this.content.data.languageStyle) {
        this.content.data.languageStyle = wwLib.wwObject.getDefault({
          type: "ww-text",
        });
      }
    ...
    async edit() {
          ...
          fields: [
            {
              ...
              desc: {
                en:
                  "Exp: html, css, javascript, json, shell, dockerfile, http, php, sql, scss, typescript",
                fr:
                  "Ex: html, css, javascript, json, shell, dockerfile, http, php, sql, scss, typescript",
              },
      ...
      try {
        ...
        if (typeof result.markdownText != "undefined") {
          this.content.data.markdownText = result.markdownText;
          this.content.data.languageStyle = result.languageStyle;
